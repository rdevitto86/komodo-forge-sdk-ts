...
- [ ] **M** `readinessHandler` accepts `Record<string, boolean>` — checks must be pre-resolved synchronously. Real dependency checks (DB, cache) are async with timeouts. Add an `asyncReadinessHandler(checks: Record<string, () => Promise<boolean>>, timeoutMs)` variant.
- [ ] **L** No `/startup` probe (Kubernetes startup probe). Add or document that liveness covers it.
