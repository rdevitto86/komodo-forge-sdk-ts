...
### Added
- **`aws/cdk/constructs` — `EventBus`.** EventBridge bus with a replayable archive (on by default, 90-day retention, pattern scoped to the deploying account) and an opt-in EventBridge Schema Registry. Schemas are supplied by the consumer as `{ schemaName, content, type }`, defaulting to `OpenApi3`, and each is made dependent on its registry so ordering is explicit rather than incidental. Callers with no finalised event shapes yet can register placeholder schema documents and replace the content without touching the construct.
- **`aws/cdk/constructs` — `RollbackAlarm`.** Blue/green rollback trigger for a live target group, built on the existing `Alarm` construct. Ships three error alarms and no performance or alert-only tier: unhealthy hosts (`>= 1`, 2 of 3 periods), target 5xx (`> 5`, 3 of 5), and — when a load balancer is passed — ELB 5xx (`> 0`, 2 of 2). All at 60s periods with `treatMissingData: NOT_BREACHING`, which is what stops an idle or low-traffic service from rolling itself back on absent datapoints. Metrics come from CDK's own `targetGroup.metrics.*` / `loadBalancer.metrics.*` helpers so metric names and dimensions are correct by construction rather than hand-written. Every threshold, evaluation period, and datapoint count is overridable; a partial override leaves the other defaults intact.
- **`aws/cdk/constructs` — `S3Bucket`.** New general-purpose bucket construct. Defaults are the secure ones: versioning on, `S3_MANAGED` encryption, `BLOCK_ALL` public access, `BUCKET_OWNER_ENFORCED` ownership, `enforceSSL`, and `RemovalPolicy.RETAIN`. Accepts lifecycle rules, server access logging, and `readRoles` / `readWriteRoles` for least-privilege grants. Previously every consumer hand-rolled `s3.Bucket` and had to remember all six security defaults.
- **`DynamoDBTable` can now create a table, not only look one up.** Supplying `partitionKey` selects create mode; omitting it preserves the existing `fromTableName` lookup behaviour exactly. Create mode supports `sortKey`, `billingMode` (default `PAY_PER_REQUEST`), `encryption` (default `AWS_MANAGED`), `timeToLiveAttribute`, `pointInTimeRecovery`, `deletionProtection`, `removalPolicy` (default `RETAIN`), `tags`, and `readWriteRoles`. Callers previously had to define stateful tables outside the SDK.
- **`ENV_CI` / `ENV_CI_FULL` constants**, plus `ci` in the `EnvConfig['env']` union, `TagsConfig['environment']`, `isValidEnvironment`, and a new `defaultCiConfig()`. `ci` is a deployed stage distinct from `dev`, which is local-only.
- **`SqsQueue` gained `enforceSSL` and `removalPolicy`.**

### Fixed
- **`SnsTopic` ignored `subscription.protocol` and subscribed everything as email.** `constructs/snsTopic.ts` accepted a `protocol` on each entry but always constructed `subscriptions.EmailSubscription`, so an HTTPS webhook or SMS subscriber was silently created as an email subscription against an endpoint that is not an email address. Protocol is now honoured for `EMAIL`, `EMAIL_JSON`, `HTTP`, `HTTPS`, and `SMS`; any protocol that cannot be built from an endpoint string (`SQS`, `LAMBDA`, …) now throws `UnsupportedSubscriptionProtocolError` instead of producing a wrong subscription. `rawMessageDelivery` is now passed through for URL subscriptions. The existing test asserted only `toBeInstanceOf`, which is why this survived.
- **`FargateService`'s 5xx alarm watched a metric that does not exist.** `constructs/fargate.ts` used `metricName: 'HTTPCode_Target_5XX'`; the published `AWS/ApplicationELB` metric is `HTTPCode_Target_5XX_Count` (per CDK's own `HttpCodeTarget.TARGET_5XX_COUNT`). Combined with `treatMissingData: NOT_BREACHING`, the alarm resolved against nothing and sat permanently in OK — the same failure mode as the dimension bug fixed above, on the same alarm. `fargate.test.ts` asserted the wrong name too, so both moved together.
- **`FargateService`'s unhealthy-target alarm needed two failures to fire.** `threshold: 1` with `GREATER_THAN_THRESHOLD` means it triggers at two or more unhealthy targets. On a service running one or two tasks, losing a single task is 50–100% of capacity and never alarmed. Now `GREATER_THAN_OR_EQUAL_TO_THRESHOLD`, so one unhealthy target fires.
- **`Alarm`'s `period` prop was silently discarded.** It was spread into `cloudwatch.AlarmProps`, which has no `period` field — CloudWatch takes period from the metric, not the alarm. Callers setting `period` got whatever the metric already carried. `period` is now applied via `metric.with({ period })`, narrowed to `Metric` and `MathExpression` since `IMetric` exposes no `with`.
- **`SqsQueue` never set the physical queue name.** `queueName` was passed as the construct id only and omitted from `sqs.QueueProps`, so every queue got a CloudFormation-generated name while the caller believed it had named it. `queueName` is now applied to the queue itself.

### Changed
- **`DynamoDBTable.table` is now `ITable | Table`** rather than `ITable`. Under this repo's `exactOptionalPropertyTypes`, no concrete CDK table class is assignable to `ITable` — both `Table` and `TableV2` declare required-but-possibly-undefined members that the interface declares optional. Lookup mode still yields `ITable`; only create mode widens the union. Consumers calling methods (`grantReadData`, `tableName`, `tableArn`) are unaffected; a consumer that assigns `.table` to an explicitly `ITable`-typed binding will need to narrow.

### Fixed (previously recorded)
- **`Alarm` silently discarded every notification action it was given.** `constructs/alarm.ts` spread `alarmActions` / `okActions` / `insufficientDataActions` into `cloudwatch.Alarm`'s props, but `cloudwatch.AlarmProps` has no such fields — only the L1 `CfnAlarm` does. Every consumer that passed a topic got an alarm that rendered with no `AlarmActions` and notified nobody, with no error at synth or deploy. The props are now applied through `cloudwatch_actions.SnsAction` and `addAlarmAction()` / `addOkAction()` / `addInsufficientDataAction()`. `constructs/alarm.test.ts` previously asserted only `toBeInstanceOf`, which is why this survived; it now asserts against the synthesized template.
- **`FargateService`'s two ALB alarms could never fire.** `constructs/fargate.ts`'s `UnhealthyTargetsAlarm` and `High5xxErrorAlarm` set the CloudWatch `LoadBalancer` dimension to `alb.loadBalancerArn`, where the `AWS/ApplicationELB` namespace requires the `app/name/id` form from `alb.loadBalancerFullName`. The dimension matched no published metric, so with `treatMissingData: NOT_BREACHING` both alarms sat permanently in OK. Every service on this construct was affected. `constructs/fargate.test.ts` asserted `MetricName` and `Namespace` but never `Dimensions`; it now asserts the resolved dimension value.
