...
describe('Integration Tests', () => {
    ...
    describe('node-worker lifecycle', () => {
        ...
        it('routes node worker ACK messages through handleAck', async () => {
            ...
            await new Promise((r) => setTimeout(r, 10));
            const msgCalls = capturedNodeWorker.on.mock.calls;
        ...
        it('node worker error event triggers degradation and falls back to fetch', async () => {
            ...
            await new Promise((r) => setTimeout(r, 10));
            const errorCalls = capturedNodeWorker.on.mock.calls;
            const [, errorHandler] = errorCalls.find(([evt]) => evt === 'error') ?? [];
            expect(errorHandler).toBeDefined();
            errorHandler(new Error('node worker crashed'));
            expect(vi.mocked(console.error)).toHaveBeenCalledWith('[komodo-logger] node worker error:', 'node worker crashed');
        ...
        it('node worker construction failure (Error) degrades to fetch', async () => {
            ...
            ref.send(makeEvent());
            await vi.waitFor(() => expect(vi.mocked(fetch)).toHaveBeenCalledOnce());
            expect(vi.mocked(console.error)).toHaveBeenCalledWith('[komodo-logger] could not start node worker:', expect.any(Error));
            wt.Worker = savedWorker;
        });
    ...
    describe('handleAck', () => {
        ...
        it('STOP directive with lost events logs to console.warn', () => {
            ...
            expect(warnSpy).toHaveBeenCalledWith('[komodo-logger] stopped with unsent logs:', expect.stringContaining('runtime: 5'));
        });
        it('STOP directive with empty lost map does not produce a console.warn', () => {
            ...
            const stoppedCalls = warnSpy.mock.calls.filter((args) => typeof args[0] === 'string' && args[0].includes('stopped with unsent'));
