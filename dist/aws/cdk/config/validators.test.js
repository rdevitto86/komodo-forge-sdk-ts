...
import { isValidARN, isValidAccount, isValidCapacity, isValidCpu, isValidDomain, isValidEnvironment, isValidMemory, isValidRegion, isValidRegionDeploy, isValidTags, isValidUpstreamDownstreamUrl, isValidVersion, validateConfig, } from './validators.js';
const validConfig = {
...
describe('isValidEnvironment', () => {
    ...
    it('rejects staging (constant is stg)', () => {
...
describe('isValidRegionDeploy', () => {
    it('accepts a valid region deploy', () => {
        expect(isValidRegionDeploy({ region: 'us-east-2', suffix: 'east', enabled: true })).toBe(true);
    });
    it('rejects an invalid region', () => {
        expect(isValidRegionDeploy({ region: 'eu-west-1', suffix: 'west', enabled: true })).toBe(false);
    });
    it('rejects missing suffix', () => {
        expect(isValidRegionDeploy({ region: 'us-east-2', enabled: true })).toBe(false);
    });
    it('rejects missing enabled', () => {
        expect(isValidRegionDeploy({ region: 'us-east-2', suffix: 'east' })).toBe(false);
    });
});
...
describe('validateConfig', () => {
    ...
    it('rejects invalid region in regions array', () => {
        expect(validateConfig(configWith({ regions: [{ region: 'eu-west-1', suffix: 'west', enabled: true }] }))).toBe(false);
    });
    ...
    it('accepts config without optional tags', () => {
        ...
        expect(validateConfig(rest)).toBe(true);
    });
    ...
    it('accepts valid upstream and downstream URLs', () => {
        expect(validateConfig(configWith({
