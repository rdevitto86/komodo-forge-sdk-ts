...
describe('isValidRegionDeploy', () => {
	it('accepts a valid region deploy', () => {
		expect(isValidRegionDeploy({ region: 'us-east-2', suffix: 'east', enabled: true } as RegionDeploy)).toBe(true);
	});
	...
	it('rejects missing suffix', () => {
		expect(isValidRegionDeploy({ region: 'us-east-2', enabled: true } as unknown as RegionDeploy)).toBe(false);
	});

	it('rejects missing enabled', () => {
		expect(isValidRegionDeploy({ region: 'us-east-2', suffix: 'east' } as unknown as RegionDeploy)).toBe(false);
	});
});
