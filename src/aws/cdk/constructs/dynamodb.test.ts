...
describe('constructs/DynamoDBTable', () => {
	...
	it('looks up the table by the given name', () => {
		expect(new DynamoDBTable(stack, 'Table', { tableName: 'my-table', taskRoles: [] }).table.tableName).toBe(
			'my-table',
		);
	});
		...
		for (const action of ['dynamodb:GetItem', 'dynamodb:Query', 'dynamodb:Scan']) {
			Template.fromStack(stack).hasResourceProperties(
				...
				Match.objectLike({
					...
					Roles: Match.arrayWith([{ Ref: stack.getLogicalId(role.node.defaultChild as cdk.CfnElement) }]),
				}),
			);
		}
	});
	...
	it('grants read data to each of multiple provided roles', () => {
		...
		template.hasResourceProperties(
			'AWS::IAM::Policy',
			Match.objectLike({
				Roles: Match.arrayWith([{ Ref: stack.getLogicalId(roleA.node.defaultChild as cdk.CfnElement) }]),
			}),
		);
		template.hasResourceProperties(
			'AWS::IAM::Policy',
			Match.objectLike({
				Roles: Match.arrayWith([{ Ref: stack.getLogicalId(roleB.node.defaultChild as cdk.CfnElement) }]),
			}),
		);
	});
