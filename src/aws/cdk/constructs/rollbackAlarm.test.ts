...
describe('constructs/rollbackAlarm', () => {
	...
	beforeEach(() => {
		...
		const vpc = new ec2.Vpc(stack, 'Vpc', { maxAzs: 2, natGateways: 0 }) as ec2.IVpc;
	...
	it('wires alarm actions to the supplied topics', () => {
		...
		Template.fromStack(stack).hasResourceProperties(
			...
			Match.objectLike({
				MetricName: 'UnHealthyHostCount',
				AlarmActions: Match.arrayWith([{ Ref: stack.getLogicalId(topic.node.defaultChild as cdk.CfnElement) }]),
			}),
		);
	});
