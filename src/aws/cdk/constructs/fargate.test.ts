...
import { FargateService, type FargateServiceProps } from './fargate.js';
...
describe('constructs/FargateService', () => {
	...
	let baseProps: FargateServiceProps;

	beforeEach(() => {
		stack = new cdk.Stack();
		vpc = new ec2.Vpc(stack, 'Vpc', { maxAzs: 2, natGateways: 0 }) as ec2.IVpc;
		cluster = new ecs.Cluster(stack, 'Cluster', { vpc }) as ecs.ICluster;
	...
	describe('deployColor traffic cutover', () => {
		it('routes default traffic to blue when deployColor is omitted', () => {
			...
			const template = Template.fromStack(stack);
			const blueTgId = stack.getLogicalId(construct.blueTargetGroup!.node.defaultChild as cdk.CfnElement);
		...
		it('routes default traffic to green when deployColor is green', () => {
			...
			const template = Template.fromStack(stack);
			const greenTgId = stack.getLogicalId(construct.greenTargetGroup!.node.defaultChild as cdk.CfnElement);
	...
	describe('capacity follows deployColor', () => {
		it('gives the green service a non-zero desired count and blue a zero desired count when deployColor is green', () => {
			...
			const template = Template.fromStack(stack);
			const blueServiceLogicalId = stack.getLogicalId(construct.blueService!.node.defaultChild as cdk.CfnElement);
			const greenServiceLogicalId = stack.getLogicalId(construct.greenService!.node.defaultChild as cdk.CfnElement);
