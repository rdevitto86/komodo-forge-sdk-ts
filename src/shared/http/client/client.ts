...
export default class HttpClient {
  ...
  async getJSON<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      if (attempt > 0) {
        await new Promise<void>(resolve => { setTimeout(resolve, 2 ** (attempt - 1) * 100); });
      }
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      ...
      try {
        const req = new Request(url, {
          ...init,
          method: 'POST',
          headers: { ...this.defaultHeaders, 'Content-Type': 'application/json', Accept: 'application/json', ...init?.headers as Record<string, string> },
          body: payload,
        });
        const resp = await this.do(req);
