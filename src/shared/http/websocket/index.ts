...
export class WebSocketClient {
  ...
  onMessage(handler: MessageHandler): this { this.onMessageHandlers.push(handler); return this; }
  onClose(handler: CloseHandler):     this { this.onCloseHandlers.push(handler);   return this; }
  onError(handler: ErrorHandler):     this { this.onErrorHandlers.push(handler);   return this; }

  get readyState(): number { return this.ws?.readyState ?? WebSocket.CLOSED; }
  get isConnected(): boolean { return this.readyState === WebSocket.OPEN; }
  ...
  private scheduleReconnect(): void {
    ...
    const delay = Math.min(
      this.opts.reconnectBaseDelayMs * 2 ** this.reconnectCount,
      this.opts.reconnectMaxDelayMs,
    );
    this.reconnectCount++;

    setTimeout(() => {
      if (!this.closed) this.connect().catch(() => { /* handled by onError */ });
    }, delay);
  }
}
