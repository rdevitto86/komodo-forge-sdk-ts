...
describe('Unit Tests', () => {
  describe('parentPort setup', () => {
    ...
    it('registers a message handler on parentPort', async () => {
      ...
      const { on } = wt.parentPort as unknown as { on: ReturnType<typeof vi.fn>; postMessage: ReturnType<typeof vi.fn> };
...
describe('Integration Tests', () => {
  describe('message routing', () => {
    it('routes an incoming LOG message through createWorkerLogic without throwing', async () => {
      ...
      if (messageHandler) {
        expect(() =>
          messageHandler({ directive: 'LOG', provider: 'ghost', payload: { msg: 'x' } })
        ).not.toThrow();
      }
    });
    ...
    it('routes an incoming FLUSH message without throwing', async () => {
      ...
      if (messageHandler) {
        expect(() =>
          messageHandler({ directive: 'FLUSH' })
        ).not.toThrow();
      }
    });
  });
