...
describe('Unit Tests', () => {
  ...
  describe('CONFIG directive', () => {
    ...
    it('starts a periodic timer when flushInterval > 0', async () => {
      ...
      await handle({ directive: 'CONFIG', provider: PROVIDER, payload: { ...BASE_CFG, flushInterval: 100, batchSize: 100 } });
    ...
    it('clears the old timer when the same provider is re-configured', async () => {
      ...
      await handle({ directive: 'CONFIG', provider: PROVIDER, payload: { ...BASE_CFG, flushInterval: 500, batchSize: 100 } });
      await vi.advanceTimersByTimeAsync(150);
  ...
  describe('STOP directive', () => {
    ...
    it('clears active timers before reporting lost events', async () => {
      vi.useFakeTimers();
      await handle({ directive: 'CONFIG', provider: PROVIDER, payload: { ...BASE_CFG, flushInterval: 1000, batchSize: 100 } });
      await handle({ directive: 'LOG', provider: PROVIDER, payload: { msg: 'buffered' } });
      await handle({ directive: 'STOP' });
  ...
  describe('createWorkerLogic', () => {
    ...
    describe('outer catch', () => {
      it('posts ERROR ack without provider when an unexpected error occurs during STOP', async () => {
        ...
        vi.stubGlobal('clearInterval', () => { throw new Error('clearInterval failed'); });
        await handle({ directive: 'STOP' });
        vi.unstubAllGlobals();
      ...
      it('includes provider on ERROR ack when the error occurs during CONFIG (has a provider)', async () => {
        ...
        vi.stubGlobal('clearInterval', () => { throw new Error('clearInterval boom'); });
        await handle({ directive: 'CONFIG', provider: PROVIDER, payload: { ...BASE_CFG, flushInterval: 200, batchSize: 100 } });
        vi.unstubAllGlobals();
    ...
    describe('periodic flush', () => {
      it('flushes buffered events automatically after flushInterval elapses', async () => {
        ...
        await h({ directive: 'CONFIG', provider: PROVIDER, payload: { ...BASE_CFG, flushInterval: 200, batchSize: 100 } });
