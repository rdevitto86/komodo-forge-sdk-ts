...
import { FargateServiceBuilder, createFargateService, type FargateServiceProps } from './fargate.js';
import { LambdaFunctionBuilder, createLambdaFunction, type LambdaFunctionProps } from './lambda.js';
...
describe('compute/fargate', () => {
  describe('FargateServiceBuilder', () => {
    ...
    it('should set task role', () => {
      const builder = new FargateServiceBuilder(mockStack, mockVpc);
      const role = new iam.Role(mockStack, 'TestRole', { assumedBy: new iam.ServicePrincipal('ecs-tasks.amazonaws.com') });
    ...
    it('should set execution role', () => {
      const builder = new FargateServiceBuilder(mockStack, mockVpc);
      const role = new iam.Role(mockStack, 'TestExecRole', { assumedBy: new iam.ServicePrincipal('ecs-tasks.amazonaws.com') });
